...
use super::{
    ...
    special::{BandcampType, Special, TwitchType},
};
...
impl Metadata {
    pub async fn from(resp: Response, url: String) -> Result<Metadata, Error> {
        ...
        let meta_selector =
            Selector::parse("meta").map_err(|_| Error::MetaSelectionFailed)?;
        ...
        let link_selector =
            Selector::parse("link").map_err(|_| Error::MetaSelectionFailed)?;

        for el in fragment.select(&link_selector) {
            let node = el.value();

            if let (Some(property), Some(content)) =
                (node.attr("rel"), node.attr("href"))
            {
        ...
        let metadata = Metadata {
            ...
            icon_url: link
                ...
                .map(|mut v| {
                    // If relative URL, prepend the root URL
    ...
    pub async fn generate_special(&mut self) -> Result<Special, Error> {
        ...
        if let Some(captures) = RE_STREAMABLE.captures_iter(&self.url).next() {
            ...
            RE_YOUTUBE.captures_iter(&self.url).next()
        {
            lazy_static! {
            ...
            if let Some(video) = &self.video {
                if let Some(timestamp_captures) =
                    RE_TIMESTAMP.captures_iter(&video.url).next()
                {
            ...
            RE_TWITCH.captures_iter(&self.original_url).next()
        {
            ...
            RE_TWITCH_VOD.captures_iter(&self.original_url).next()
        {
            return Ok(Special::Twitch {
                id: captures[1].to_string(),
            ...
            RE_TWITCH_CLIP.captures_iter(&self.original_url).next()
        {
            return Ok(Special::Twitch {
                id: captures[1].to_string(),
            ...
            RE_SPOTIFY.captures_iter(&self.original_url).next()
        {
            ...
            lazy_static! {
                static ref RE_ALBUM: Regex =
                    Regex::new("album=(\\d+)").unwrap();
                static ref RE_TRACK: Regex =
                    Regex::new("track=(\\d+)").unwrap();
            }

            if let Some(video) = &self.video {
                if let Some(captures) =
                    RE_TRACK.captures_iter(&video.url).next()
                {
                ...
                if let Some(captures) =
                    RE_ALBUM.captures_iter(&video.url).next()
                {
                    return Ok(Special::Bandcamp {
                        id: captures[1].to_string(),
    ...
    pub async fn resolve_external(&mut self) {
        if let Ok(special) = self.generate_special().await {
            match &special {
                Special::YouTube { .. } => {
                    self.color = Some("#ff424f".to_string())
                }
                Special::Twitch { .. } => {
                    self.color = Some("#7b68ee".to_string())
                }
                Special::Spotify { .. } => {
                    self.color = Some("#1abc9c".to_string())
                }
                Special::SoundCloud { .. } => {
                    self.color = Some("#ff7f50".to_string())
                }
                _ => {}
            }
    ...
    async fn resolve_image(&mut self) -> Result<(), Error> {
        if let Some(image) = &mut self.image {
            // If the image's height and width was already provided by the Open
            // Graph tags, just return that instead
